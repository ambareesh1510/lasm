        ...
        match self {
            Filetype::EncodedBinary(s) => {
                ...
                for i in 0..(input_bytes.len() / 2) {
                    return_bytes[i + 0x3000] = (input_bytes[2 * i] as u16 * 2u16.pow(8)
                        + input_bytes[2 * i + 1] as u16)
                        as i16;
                }
                Ok(return_bytes)
            }
            Filetype::PlaintextBinary(s) => {
                let input_bytes = fs::read_to_string(s)
                    .unwrap()
                    .trim()
                    .split_whitespace()
                    .collect::<String>();
                ...
                for i in 0..(input_bytes.len() / 16) {
                    return_bytes[i + 0x3000] =
                        (u16::from_str_radix(&input_bytes[i * 16..(i + 1) * 16], 2).unwrap())
                            as i16;
                }
                Ok(return_bytes)
            }
